...
3. Translate the use case into a persona blueprint before any prospect fetch.
   ...
   **Bare query (no use case, no function override) — default to executives only.** When the user supplies only a company identifier ("contact shortlist for snowflake.com") with no use case, function, or persona hint, default to a tight executive cohort. Use `job_level: ["c-suite", "president", "vice president", "owner"]` — this covers (a) C-suite officers at all sizes (CEO/CFO/COO/CTO/CRO/CMO/CIO/CISO/Chief X), (b) President + EVP, (c) **VPs**, who are the actual buyer at most large enterprises where C-suite is unreachable, and (d) **Owners** at SMB / private companies where the founder-owner is the operational decision-maker. Return 5–10 names — under-specified intent maps more naturally to "who runs this place" than to a 25-row buying-committee spread. Director-level and below are deliberately NOT in the default cohort — they're what users *narrow to* when they specify a function, not what they want as a default. Add a one-line nudge to the output: *"Want broader buying-committee coverage (Directors + Managers across functions)? Specify a use case (prospecting / deal acceleration / renewal-expansion) or function (sales / engineering / RevOps / security / finance)."*
...
- **Bare-query default is the executive cohort** (`c-suite + president + vice president + owner`, 5–10 names) — NOT the broader buying-committee spread that includes Directors. Earlier defaults (either "prospecting buying-committee" or "c-suite + president only") were both wrong: the former too broad, the latter too narrow (excluded the VPs who do the actual buying at most enterprises and the Owners who are the operational decision-maker at SMBs). Users seeking the wider Director/Manager spread must specify a use case or function explicitly. Tight executive cohorts also tenure-gate cleaner because the row count is small enough to verify every name.
