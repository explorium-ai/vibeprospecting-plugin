...
2. Resolve each lead. Bucket every input row; never silently drop one. Known prospect IDs pass through. Email rows resolve by email: email is a unique identifier, so a no-match is a hard fail, NOT a fallback to name search (a typo'd email must not silently resolve to a different real person). LinkedIn URL rows resolve by URL. Name+company rows resolve by full name plus company name or domain: person matching returns only an id or null (no confidence score, no candidate list), so a name+company match at a 10k+ employee company is indistinguishable from a unique email match. Treat name+company matches as Verified-with-caveat by default. Free-text rows like "Jane Doe at Acme" parse to the name+company path. Persona-only rows (no name, "the CFO at Notion") must NOT use full_name="CFO" (silently returns nothing); instead match the business, then sample prospects at that account filtered by canonical job title and seniority, and surface the candidates as Verified (caveat) for the user to pick. **If the candidate set is empty after broadening to `department=null + job_level=c-suite`** (because Chief X roles surface as Unattributed per Limitations), tag the row `Failed (persona-not-instantiated)` and surface the matched `business_id` so the caller can manually pick a substitute persona. **Do NOT emit an empty Verified row** — that masks a real coverage gap as a successful resolution. Tag each row: Auto-resolved, Verified (caveat), Ambiguous (pause), Failed (no match), or Failed (persona-not-instantiated). Never reroute Failed emails into name search.
...
- **Field map (API source for each evidence column):**
  - `mapped seniority` → `profile_job_seniority_level`
  - `mapped department` → `profile_job_department_main` (fallback to `profile_job_department_array[0]` when `_main` is null)
  - `event age in days` → `now - event_time` (caller computes; not a returned field)
  - `top 3 events` → highest-priority 3 from `fetch-businesses-events` response, sorted by recency. Preview cap is 3, export cap is 50 — these are different and the caller must not conflate them.
  - `tech stack hits` → intersection of `firmo_techno_*` arrays with the ICP tech list
  - `headcount`, `revenue range`, `country`, `industry`, `size bucket`, `revenue bucket` → `firmo_*` fields on the resolved business
...
- **Name+company resolution at large companies — status tagging rule (mandatory).** The match API returns one `prospect_id` or null with no confidence signal, so a null at a 10,000-employee company is indistinguishable from a unique-id match. Reconcile via headcount band:
  - At companies with `firmo_number_of_employees_range` in `[1001-5000, 5001-10000, 10001+]`: `prospect_id` returned → `Verified (caveat-large-co)`; `prospect_id` null → `Failed (no-unique-match)`.
  - At sub-1000-headcount companies: `prospect_id` returned → `Verified`; null → `Failed`.
  - **`Ambiguous` is reserved for cases where the caller supplied multiple candidate identities** and the matcher cannot disambiguate between them — NOT for single-input/null-output. The earlier "Ambiguous (pause)" framing for null-output rows was wrong; null-output is `Failed`, not `Ambiguous`.
...
- **Output columns map to specific API fields** — see the "Field map" sub-section under Output Format. `mapped department` → `profile_job_department_main` (NOT the plain `profile_job_department`, which is often null even when the canonical value is present in `_main`). `event age in days` is computed locally from `event_time`; it is not a returned field.
- **Persona-only failures (`Failed (persona-not-instantiated)`) signal that the named seat is not findable at this account via title+seniority filters**, even after broadening to `department=null + job_level=c-suite`. This is a real coverage gap — not a default that should silently become a Verified row. Caller should manually pick a substitute persona or skip the lead.
