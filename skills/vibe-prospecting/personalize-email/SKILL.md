...
1. Resolve the prospect. If a LinkedIn URL or email is supplied, match the person directly. Otherwise match the business first to anchor the company, then match the person by full name plus the resolved company. On miss, retry first plus last name. Capture prospect_id, business_id, full_name, job_title, linkedin_url. If either id fails, stop and surface the gap.
   ...
   **Name sanitization (mandatory before any name-based match call).** `match-prospects` `full_name` rejects apostrophes and most non-alphanumeric punctuation with an opaque regex error — names like `O'Brien`, `D'Angelo`, `de la Cruz`, `Müller` all break the call silently if passed raw. Strip apostrophes (`D'Angelo` → `DAngelo`) before the first match call; preserve hyphens and accented characters. On the documented retry path, try BOTH the sanitized and the original strings. If the input contains characters outside `[a-z¡-￿\s.-]` (case-insensitive), treat the regex error as a normalization issue, not a not-found result — do not assume "no match" until the sanitized variants have been tried.
2. Resolve-by-role path (the user named a seat, not a person). Matching by full_name="CFO" silently returns nothing: do NOT use that path. Instead match the business, then sample prospects at that account filtered by canonical job title AND seniority. **Filter discipline (mandatory):** matching by `job_title` ALONE returns broad results that include "Executive Assistant to the CFO" and similar near-miss titles — burning credits and surfacing the wrong person. ALWAYS combine `job_title` with `job_level` (`c-suite` for CXO seats, `vice president` / `director` for VPs and directors). When targeting executives, also exclude titles containing "assistant" or "to the" via the `negate` flag. Pick the top 2-3 autocomplete canonical variants, not all 10 — the long tail (e.g. "controller / chief financial officer") is noise. This is the dominant cold-outbound shape when the user knows the seat but not the person.
...
6. Fetch business events on the last 90 days. Keep every event with a date; drop those older than the use-case recency floor during scoring. **Event-attribution sanity check (extended — non-optional):**
   (a) **Parent/subsidiary blending:** events from an Adani subsidiary attaching to "Adani Group" — flag and assess relevance.
   (b) **Competitor-mention contamination:** events that name the matched company only as a competitor or comparison point, not a participant. For each candidate event, verify that `data.companies_involved` contains the matched business — not just `data.title` or `data.snippet`. If the matched business appears ONLY in `snippet` text (e.g. "Amazon buys Globalstar to compete with SpaceX's Starlink" attached to SpaceX), DROP the event and flag `competitor_mention_detected: true`. Otherwise the calling model will hallucinate "congrats on the new acquisition" emails to companies that did not acquire anything.
...
- **Role-only resolution requires `job_title` + `job_level` paired filters AND `assistant`/"to the" exclusion.** Step 2 mandates this. Title-only resolution silently includes Executive Assistants, burning credits and surfacing the wrong person — the single biggest source of wasted credits in this workflow when the user names a seat not a person.
- **Event attribution must verify `data.companies_involved`, not just `data.title` / `data.snippet`.** Competitor-mention contamination (the matched company appears only in snippet text as a competitor or comparison point, not as a participant) silently anchors fabricated "congrats on the acquisition" emails. The extended attribution check in step 6 is non-optional.
- **`match-prospects` `full_name` regex rejects apostrophes** and other non-alphanumeric punctuation with an opaque error. Name sanitization in step 1 (strip apostrophes, preserve hyphens and accented characters, try both sanitized and original on retry) is mandatory before any name-based match call.
